...
export default function CalendarApp() {
  ...
  const renderCalendarDays = () => {
    ...
    for (let i = 0; i < firstDay; i++) {
      days.push(<div key={`empty-${i}`} className="h-24 border border-gray-200 dark:border-gray-600"></div>);
    }
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <div
          key={day}
          className={`h-24 border border-gray-200 dark:border-gray-600 p-1 cursor-pointer hover:bg-blue-50 dark:hover:bg-blue-900/20 ${
            isToday ? 'bg-blue-100 dark:bg-blue-900/30' : ''
          }`}
          onClick={() => selectDate(day)}
        >
          <div className={`font-semibold ${isToday ? 'text-blue-600 dark:text-blue-400' : 'text-gray-800 dark:text-gray-200'}`}>
            {day}
          </div>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {showEventForm && selectedDate && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-96 max-w-90vw transition-colors">
              ...
              <input
                ...
                className="w-full p-3 border border-gray-300 dark:border-gray-600 rounded mb-4 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400"
              ...
              {events[formatDateKey(selectedDate)]?.length > 0 && (
                <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-600">
                  <h4 className="font-semibold mb-2 text-gray-800 dark:text-gray-100">Existing Events:</h4>
                  {events[formatDateKey(selectedDate)].map((event, index) => (
                    <div key={index} className="bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 p-2 rounded mb-2">
                      {event}
                    </div>
                  ))}
                </div>
