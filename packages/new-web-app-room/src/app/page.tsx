...
export default function CalendarApp() {
  ...
  const renderCalendarDays = () => {
    ...
    for (let i = 0; i < firstDay; i++) {
      days.push(<div key={`empty-${i}`} className="h-24 border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800"></div>);
    }
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <div
          key={day}
          className={`h-24 border border-gray-200 dark:border-gray-600 p-1 cursor-pointer hover:bg-blue-50 dark:hover:bg-gray-700 bg-white dark:bg-gray-800 transition-colors ${
            isToday ? 'bg-blue-100 dark:bg-blue-900' : ''
          }`}
          onClick={() => selectDate(day)}
        >
          <div className={`font-semibold ${isToday ? 'text-blue-600 dark:text-blue-300' : 'text-gray-900 dark:text-gray-100'}`}>
            {day}
          </div>
