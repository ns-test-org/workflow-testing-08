...
export default function CalendarApp() {
  ...
  const renderCalendarDays = () => {
    ...
    for (let i = 0; i < firstDay; i++) {
      days.push(<div key={`empty-${i}`} className="h-24 border border-gray-200 dark:border-gray-600 dark:bg-gray-800"></div>);
    }
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <div
          key={day}
          className={`h-24 border border-gray-200 dark:border-gray-600 p-1 cursor-pointer hover:bg-blue-50 dark:hover:bg-gray-700 dark:bg-gray-800 transition-colors ${
            isToday ? 'bg-blue-100 dark:bg-blue-900' : ''
          }`}
          onClick={() => selectDate(day)}
        >
          <div className={`font-semibold ${isToday ? 'text-blue-600 dark:text-blue-400' : 'dark:text-gray-100'}`}>
            {day}
          </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-96 max-w-90vw">
              ...
              <input
                ...
                className="w-full p-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded mb-4 focus:outline-none focus:ring-2 focus:ring-blue-500"
              ...
              {events[formatDateKey(selectedDate)]?.length > 0 && (
                <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-600">
                  <h4 className="font-semibold mb-2 dark:text-gray-100">Existing Events:</h4>
                  {events[formatDateKey(selectedDate)].map((event, index) => (
                    <div key={index} className="bg-gray-100 dark:bg-gray-700 dark:text-gray-100 p-2 rounded mb-2">
                      {event}
                    </div>
                  ))}
                </div>
